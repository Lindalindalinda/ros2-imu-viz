...
class ImuSubscriberForPyBullet(Node):
    ...
    def update_imu_data_for_pybullet(self, msg):
        try:
            ...
            p.resetBasePositionAndOrientation(self.q, [0, 0, 0], [msg.orientation.x, msg.orientation.y, msg.orientation.z, msg.orientation.w])
            
            self.get_logger().info('imu_sub_pybullet:: updated orientation.')

            acc = msg.linear_acceleration
            self.get_logger().info(f'imu_sub_pybullet:: got LinearAcc::{acc}')
            # p.setGravity(acc.x, acc.y, acc.z)
        except:
            logging.exception("error occurred")
            pass
